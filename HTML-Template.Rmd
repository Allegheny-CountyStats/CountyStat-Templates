...
# Connect to the PanDA DataWarehouse
...
The best functions for tables are `DT()` and `kable()`. The best option will depend on the number of rows and situation you find yourself in. Both of these functions allow for a wide amount of customization and formating. Performance and Data Analytics (PanDA) has a standard table format to start with, however Analysts should feel free to check out the documentation for both packages especially when looking to format the rows to highlight certain values or increase readability, or improve other functions.

### DT

DT has extensive [documentation](https://rstudio.github.io/DT/) showing its features. Below is the standard format for a PanDA DT table. One of the biggest benefits to this package is the [horizontal scroll option](https://stackoverflow.com/questions/30765338/how-to-make-the-horizontal-scrollbar-visible-in-dtdatatable) and [download button](https://rstudio.github.io/DT/extensions.html) feature.
