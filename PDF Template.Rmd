...
[ggmap](https://github.com/dkahle/ggmap) works if you are not trying to have interactive elements. Like leaflet the `Stamen.Toner` base map is suggested. You can use `plotly` on a typical `geom_sf`, but in most cases `leaflet` is a better option.
...
get_stamenmap(bbox_trans, maptype = "toner-lite") %>% 
  ggmap() +
  theme_map() +
  geom_sf(data = alco_muni_pop, aes(fill = estimate), inherit.aes = FALSE) +
